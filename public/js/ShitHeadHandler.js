class ShitHeadHandler extends GameScene {
    ...
    preload() {
        super.preload();
        this.load.spritesheet("cards", "/img/cards2.png", {
            frameWidth: 150,
            frameHeight: 200
        });
    ...
    createStacksForPlayer(player) {
        ...
        {
            player.playerNameText = this.add.text(0.02 * this.game.config.width, 0.62 * this.game.config.height, player.name, {
                fixedWidth: 0.5 * this.game.config.width,
                align: "left",
                fontSize: 16,
                fontFamily: "Pacifico"
        ...
        {
            player.playerNameText = this.add.text(0.25 * this.game.config.width, 0.13 * this.game.config.height, player.name, {
                fixedWidth: 0.5 * this.game.config.width,
                align: "center",
                fontSize: 16,
                fontFamily: "Pacifico"
        ...
        {
            player.playerNameText = this.add.text(0.5 * this.game.config.width, 0.62 * this.game.config.height, player.name, {
    ...
    onGameStateChange(newState) {
        ...
        if (newState === "startOfGame") {
            ...
            throwStack.onAddedCardToTop = (newCard) => {
                ...
                const burn = newCard.cardValue === 10 || throwStack.areTopCardsSameValue(4) || (newCard.cardType === JOKER); // if the top 4 cards are the same, or a 10 is thrown, burn it
                if (burn) {
                    console.log("BURN!!");
                ...
                // else if (newCard.cardValue === 6)
                // {
                //      this.previouslyThrownValueThisRound =  this.underlayingCard;
                //       console.log("6 was put")
                //     
                //    }
                //
                else if (newCard.cardValue === 3) {
                    this.previouslyThrownValueThisRound = null;
                }
    ...
    canPlay(card, player) { // <-- only called if the local player relocates a card
        ...
        var cards = throwStack.containingCards.filter((c) => c.cardValue !== 6); // <-- remove all sevens because they are transparent
        ...
        } else if (cardValue === 6 || card.cardType === JOKER || cardValue === 3 || cardValue === 10 || cardValue === underlayingValue) // these cards can be thrown all the time
        {
            console.log("mayCardBeThrown()", "6 or JOKER");
            return true;
        }
        ...
        //      return cardValue <= 9;
        //  }
        else if (cardValue === 3) // these cards can be thrown on all but the joker
        {
            console.log("mayCardBeThrown()", "3");
            return true;
        ...
        } else if (underlayingValue === 3) {
            console.log("mayCardBeThrown()", "3");
            return underlayingCard = 1;

        } else if (cardValue === underlayingCard) {
            console.log("mayCardBeThrown()")
            return true;
